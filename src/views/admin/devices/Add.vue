<template>
  <div id="add-view">
    ...
    <v-sheet width="650" height="90vh" class="pa-4 mx-auto d-flex flex-column justify-center">
      <div class="text-h5 mb-3">{{ this.id ? "Edit" : "Add" }} Device</div>
      <v-form ref="signUpForm" lazy-validation>
        <v-row>
          <v-col cols="12" sm="12" md="12" class="pb-0">
            <v-text-field v-model="name" :rules="deviceNameRules" required label="Device Name" outlined dense />
          </v-col>
          <v-col cols="12" sm="12" md="12" class="pb-0">
            <v-text-field v-model="description" label="Description" outlined dense />
          </v-col>
        </v-row>
        <v-btn class="float-right" color="primary" @click="handleSavingDevice" depressed>Save</v-btn>
        <v-btn class="float-right mr-1" color="secondary" to="/admin/device/list" depressed>Cancel</v-btn>
      </v-form>
    </v-sheet>
  </div>
</template>
...
export default {
  ...
  async mounted() {
    if (this.id) {
      const response = await adminApi.get(`/api/device/${this.id}`, {
        headers: {
    ...
    async handleSavingDevice() {
      ...
      try {
        ...
        if (response.data.success) {
          ...
          setTimeout(() => {
            const actionName = this.id ? "updateDevice" : "addDevice";
